...
public class ActeurActeAssureurDAO implements ConstantesMetier {
	
	private static Logger log = Logger.getLogger(ActeurActeAssureurDAO.class);
	private static final ActeurActeAssureurDAO INSTANCE = new ActeurActeAssureurDAO();

	public static ActeurActeAssureurDAO getInstance() {
		  return INSTANCE;
		 }

	private ActeurActeAssureurDAO () {}
	public ActeurActeAssureur getActeurActeAssureur(String acteurActeAssureurId)
	{
		     ...
		     acteurActeAssureur = (ActeurActeAssureur) session.get(ActeurActeAssureur.class, acteurActeAssureurId);
	...
	public ActeurActeAssureur getActeurActeAssureurByDate(Acteur acteur,Date date1,Acte acte,Categorie categorie)
	{
		     ...
		     try
		     {
		    	   if (acteur!=null && date1!=null && acte!=null && categorie!=null)
		    	   {
			    	...
			    	strQuery+=" where acteurActeAssureur.acte.acteId = "+acte.getActeId();
			    	strQuery+=" and acteurActeAssureur.acteur.acteurId = "+acteur.getActeurId();
			    	strQuery+=" and acteurActeAssureur.categorie.categorieId = "+categorie.getCategorieId();
			    	strQuery+=" and acteurActeAssureur.isException = "+STATUT_VALIDE;
			    	strQuery+=" and acteurActeAssureur.statut = "+STATUT_VALIDE;
			    	strQuery+=" and acteurActeAssureur.dateDebut <= '"+ dateFacture+"'";
			    	strQuery+=" and acteurActeAssureur.dateFin >= '"+ dateFacture+"'";
	...
	public void updateActeurActeAssureur(ActeurActeAssureur acteurActeAssureur)
	{
             log.debug("********** Debut updateActeurActeAssureur ActeurActeAssureurDAO **********");
		     try
		     {
		     Session session = SessionFactoryUtil.getInstance().openSession();
		     ...
		     {
	             log.debug("********** Fin updateActeurActeAssureur ActeurActeAssureurDAO **********");
		     }
	}
	...
	public List<ActeurActeAssureur> listActeurActeAssureursActeurActeAssureururs() 
	{
		...
		try 
	    {
	    	...
	    	strQuery+="and acteurActeAssureur.statut = "+STATUT_VALIDE;
	    	strQuery+=" and acteurActeAssureururActeurActeAssureur.statut = "+STATUT_VALIDE;

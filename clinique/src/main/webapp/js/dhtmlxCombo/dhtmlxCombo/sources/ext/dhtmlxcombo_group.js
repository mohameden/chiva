	...
	this.attachEvent("onChange",function(){
		...
		if(that.getActualValue()=="") { 
			...
			if(_chcombo._xml){
				...
				_chcombo._xml += ((_chcombo._xml.indexOf("?")!=-1)?"&":"?")+"parent="+encodeURIComponent(that.getActualValue());			
			}
			else{
				if(xml){
					_chcombo.clearAll(true);
					_chcombo.loadXML(xml+((xml.indexOf("?")!=-1)?"&":"?")+"parent="+encodeURIComponent(that.getActualValue()));
				}

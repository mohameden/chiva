...
@Repository
public class EntreeStockDAO extends CliniqueHibernateDaoSupport<EntreeStock> {
	...
	public EntreeStock getEntreeStock(int entreeStockId) {
		...
		try {
			...
			entreeStock = (EntreeStock) session.get(EntreeStock.class,
					entreeStockId);
			return entreeStock;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public EntreeStock getEntreeStockByProduitAndDateAndFournisseur(
			int produitId, Date dateEntree, int fournisseurId) {
		...
		try {
			...
			strQuery += " and entreeStock.fournisseur.fournisseurId = "
					+ fournisseurId;

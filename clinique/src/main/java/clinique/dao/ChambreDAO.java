...
@Repository
public class ChambreDAO extends CliniqueHibernateDaoSupport<Chambre> {
	...
	public List<Chambre> listChambresLibres() {
		...
		try {
			...
			chambres = session.createQuery(strQuery).setCacheable(false).list();
			/*
			 * chambres =session.createCriteria(Chambre.class) .add(
			 * Expression.like("statut", STATUT_VALIDE) ) .add(
			 * Expression.like("etat", ETAT_CHAMBRE_LIBRE) ) .list();
			 */
			return chambres;
		} catch (Exception e) {
			e.printStackTrace();

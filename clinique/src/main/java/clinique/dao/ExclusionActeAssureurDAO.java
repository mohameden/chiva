...
@Repository
public class ExclusionActeAssureurDAO extends
		CliniqueHibernateDaoSupport<ExclusionActeAssureur> {

	private static Logger log = Logger
			.getLogger(ExclusionActeAssureurDAO.class);

	public ExclusionActeAssureur getExclusionActeAssureur(
			String exclusionActeAssureurId) {
		log.debug("********** Debut getExclusionActeAssureur ExclusionActeAssureurDAO **********");
		try {
			ExclusionActeAssureur exclusionActeAssureur = null;
			Session session = getSession();
			exclusionActeAssureur = (ExclusionActeAssureur) session.get(
					ExclusionActeAssureur.class, exclusionActeAssureurId);
	...
	public ExclusionActeAssureur getExclusionActeAssureurByActe(Acte acte) {
		...
		try {
			...
			strQuery += " where exclusionActeAssureur.statut = "
					+ STATUT_VALIDE;
			strQuery += " and exclusionActeAssureur.acte.acteId = "
					+ acte.getActeId();
			strQuery += " order by exclusionActeAssureur.exclusionId asc";
			org.hibernate.Query query = session.createQuery(strQuery);
			exclusionActeAssureurs = query.list();

			if (exclusionActeAssureurs.size() == 0) {
				return null;
			} else {
				return exclusionActeAssureur = exclusionActeAssureurs.get(0);
			}
		} catch (Exception e) {
			e.printStackTrace();
			log.fatal(e.getMessage());
	...
	public void saveExclusionActeAssureur(
			ExclusionActeAssureur exclusionActeAssureur) {
	...
	public void updateExclusionActeAssureur(
			ExclusionActeAssureur exclusionActeAssureur) {
	...
	public void deleteExclusionActeAssureur(
			ExclusionActeAssureur exclusionActeAssureur) {
	...
	protected Class<?> getEntityClass() {
		return ExclusionActeAssureur.class;
	}

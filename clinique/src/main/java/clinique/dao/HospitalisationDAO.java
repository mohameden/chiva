...
@Repository
public class HospitalisationDAO extends
		CliniqueHibernateDaoSupport<Hospitalisation> {
	...
	public Hospitalisation getHospitalisation(String hospitalisationId) {
		...
		try {
			...
			hospitalisation = (Hospitalisation) session.get(
					Hospitalisation.class, hospitalisationId);
			...
			if (hospitalisation != null) {
				...
				System.out.println("Nooooooooooooookkkkkkkkkkkkkk");
			}
	...
	public Hospitalisation getHospitalisationById(String hospitalisationId) {
		...
		try {
			...
			Session session = getSession();
			// hospitalisation = (Hospitalisation)
			// session.get(Hospitalisation.class, hospitalisationId);

			String strQuery = "from Hospitalisation h where h.hospitalisationId=";
			strQuery += hospitalisationId;
			List list = session.createQuery(strQuery).setCacheable(false)
					.list();
			if (list.size() > 0) {
				hospitalisation = (Hospitalisation) list.get(0);
			}
			return hospitalisation;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Hospitalisation> listHospitalisationsEncours() {
		...
		try {
			...
			strQuery += " or hospitalisation.encours = "
					+ STATUT_HOSPITALISATION_NO_REGLE;
			strQuery += " ) order by hospitalisation.hospitalisationId asc";
			org.hibernate.Query query = session.createQuery(strQuery);
			hospitalisations = query.setCacheable(false).list();

			int i = 0;

			for (Object element2 : hospitalisations) {

				Hospitalisation element = (Hospitalisation) element2;

				String id = element.getHospitalisationId();

				element = (Hospitalisation) session.get(Hospitalisation.class,
						id);
